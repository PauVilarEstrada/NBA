          ...
          <motion.div key="setup" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <GlassCard>
              <div className="grid gap-5 lg:grid-cols-[1fr_auto_1fr]">
                <TeamSlot team={away} onChange={setAway} label={t.predictTeam.awayTeam} side="away" />
                <div className="flex flex-col items-center justify-center gap-3">
                  ...
                  <button
                    ...
                    {t.simulate.swapHomeCourt}
                  </button>
                  <Toggle checked={isPlayoffs} onChange={setIsPlayoffs} label={t.common.playoffs} />
                </div>
                <TeamSlot team={home} onChange={setHome} label={t.predictTeam.homeTeam} side="home" />
              </div>

              {sameTeam && (
                <p className="mt-4 text-center text-sm" style={{ color: 'var(--bad)' }}>
                  {t.simulate.pickDifferent}
                </p>
              )}

              {preview && (
                <div className="mt-6 border-t pt-4" style={{ borderColor: 'var(--border)' }}>
                  <p className="eyebrow mb-2">{t.simulate.analyticalForecast}</p>
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
                    {([
                      [t.simulate.projectedScore, `${f.dec(preview.projection.homePts.mean)} – ${f.dec(preview.projection.awayPts.mean)}`],
                      [t.predictTeam.spread, f.signed(preview.projection.spread)],
                      [t.predictTeam.total, f.dec(preview.projection.total.mean)],
                      [t.predictTeam.possessions, f.dec(preview.projection.possessions)],
                    ] as const).map(([k, v]) => (
            ...
            {home && away && !sameTeam && (
              <div className="grid gap-4 lg:grid-cols-2">
                {[away, home].map((team) => (
                  <GlassCard key={team.abbr} accent={team.primaryColor}>
                    <SectionTitle
                      title={t.simulate.availability(team.abbr)}
                      sub={t.simulate.availabilitySub}
                      right={<TeamLogo teamId={team.teamId} abbr={team.abbr} size={28} />} />
                    <ul className="space-y-1.5">
                      {engine.roster(team.abbr).map((p) => (
                        <AvailabilityRow key={p.playerId} player={p} team={team}
                          isOut={out.has(p.playerId)} onToggle={() => toggleOut(p.playerId)} />
                      ))}
                    </ul>
                    <RosterImpact team={team} out={out} />
                  </GlassCard>
                ))}
              </div>
            )}
            ...
            <GlassCard>
              <div className="flex flex-wrap items-end justify-between gap-4">
                <div className="flex flex-wrap items-center gap-4">
                  <div>
                    <p className="eyebrow mb-1.5">{t.simulate.confidenceSweep}</p>
                    <Segmented value={runs} onChange={setRuns} options={[
                      { value: 0, label: t.simulate.off }, { value: 100, label: t.simulate.runs(100) },
                      { value: 200, label: t.simulate.runs(200) }, { value: 500, label: t.simulate.runs(500) },
                    ]} />
                  </div>
                  <div>
                    <p className="eyebrow mb-1.5">{t.common.seed}</p>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button onClick={() => run()} disabled={!canRun}
                    ...
                    {isPlayoffs ? t.simulate.playPlayoffGame : t.simulate.playGame}
                  </button>
                </div>
              </div>
        ...
        <motion.div key={team.abbr}
          ...
          <div className="min-w-0">
            ...
            <p className="num text-[11px]" style={{ color: 'var(--text-muted)' }}>
              {team.wins}-{team.losses} · {t.abbr.off} {f.dec(team.offRating)} · {t.abbr.def}{' '}
              {f.dec(team.defRating)} · {t.abbr.pace} {f.dec(team.pace)}
            </p>
          </div>
      ...
      <button onClick={onToggle}
        ...
        <span className="min-w-0 flex-1">
          ...
          <span className="text-[10px]" style={{ color: 'var(--text-muted)' }}>
            {player.position} · {f.dec(player.min)} {t.abbr.min.toLowerCase()}
          </span>
        </span>
        <span className="num text-xs" style={{ color: 'var(--text-2)' }}>
          {f.dec(player.pts)} / {f.dec(player.reb)} / {f.dec(player.ast)}
        </span>
        <span className={clsx('shrink-0 rounded-md px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wider')}
          ...
          {isOut ? t.common.out : t.common.active}
        </span>
      </button>
    </li>
  )
...
function RosterImpact({ team, out }: { team: Team; out: Set<number> }) {
  ...
  return (
    <div className="mt-3 border-t pt-3" style={{ borderColor: 'var(--border)' }}>
      <p className="eyebrow mb-2">{t.simulate.effectOn(team.abbr)}</p>
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="rounded-lg px-2 py-1.5" style={{ background: 'var(--layer-1)' }}>
          <p className="text-[9px] uppercase tracking-widest" style={{ color: 'var(--text-muted)' }}>{t.predictTeam.offence}</p>
          <p className="num text-sm font-bold" style={{ color: 'var(--bad)' }}>
            {f.dec(team.offRating)} → {f.dec(team.offRating - offPenalty)}
          </p>
        </div>
        <div className="rounded-lg px-2 py-1.5" style={{ background: 'var(--layer-1)' }}>
          <p className="text-[9px] uppercase tracking-widest" style={{ color: 'var(--text-muted)' }}>{t.predictTeam.defence}</p>
          <p className="num text-sm font-bold" style={{ color: 'var(--bad)' }}>
            {f.dec(team.defRating)} → {f.dec(team.defRating + defPenalty)}
          </p>
        </div>
        <div className="rounded-lg px-2 py-1.5" style={{ background: 'var(--layer-1)' }}>
          <p className="text-[9px] uppercase tracking-widest" style={{ color: 'var(--text-muted)' }}>{t.simulate.outCount}</p>
