...
type SortKey = 'pick' | 'pts' | 'roy' | 'per'

/** The score components below are built with English labels so the model reads
 *  clearly; the dictionary owns the copy, so each is mapped here at render time
 *  and falls back to the shipped label when there is no key for it. */
...
export default function Rookies() {
  ...
  const SORTS: Array<{ value: SortKey; label: string }> = [
    { value: 'pick', label: t.rookies.sortPick },
    { value: 'pts', label: t.stat.pts },
    { value: 'roy', label: t.rookies.sortRoy },
    { value: 'per', label: t.stat.per },
  ]
  ...
  if (!rows) {
    return (
      <PageTransition>
        <SectionTitle title={t.rookies.loadingTitle} sub={t.rookies.loadingSub} />
    ...
    <PageTransition>
      ...
      <section className="relative overflow-hidden rounded-3xl p-6 sm:p-8"
              ...
              <div className="mt-2 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="num text-xs" style={{ color: 'var(--text-muted)' }}>
                    {leader.p.team}
                    {leader.p.bio.draftPick ? ` · ${t.rookies.pickShort(leader.p.bio.draftPick)}` : ''}
                    {' · '}{f.dec(leader.p.pts)} {t.abbr.ppg}
                  </p>
                </div>
              </div>
            </Link>
        ...
        <div className="relative mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
          <HeroTile label={t.rookies.tracked} value={rows.length} decimals={0} />
          <HeroTile label={t.rookies.topScorer} value={topScorer?.pts ?? 0} suffix={` ${t.abbr.ppg}`}
            caption={topScorer?.name} />
          <HeroTile label={t.rookies.avgMinutes}
            value={rows.reduce((s, p) => s + p.min, 0) / rows.length} suffix=" min" />
          <HeroTile label={t.rookies.classPayroll}
      ...
      <section className="mt-6 grid gap-4 lg:grid-cols-2">
        <ChartFrame
          title={t.rookies.royRace}
          sub={t.rookies.royRaceSub}
          height={320}
          table={{
            columns: [t.common.rookie, t.abbr.team, t.abbr.score, t.abbr.pts, t.abbr.min, t.abbr.ts],
            rows: royRace.map(({ p, roy }) => [
              p.name, p.team, f.dec(roy.total), f.dec(p.pts), f.dec(p.min), f.pct(p.ts, 1),
            ]),
          }}
        >
              ...
              <Tooltip cursor={{ fill: 'var(--layer-1)' }}
                content={({ active, payload, label }) => active && payload?.length ? (
                  <TooltipBox title={String(label)} rows={[{
                    label: t.rookies.royScore, value: f.dec(Number(payload[0].value)),
                  }]} />
                ) : null} />
        ...
        <GlassCard>
          <SectionTitle title={t.rookies.royBreakdown} sub={t.rookies.royBreakdownSub} />
            ...
            {royRace.slice(0, 5).map(({ p, roy }, i) => (
              <div key={p.playerId} className="rounded-xl p-3" style={{ background: 'var(--layer-1)' }}>
                <div className="mb-2 flex items-center gap-2">
                  ...
                  <span className="num text-base font-bold">{f.dec(roy.total)}</span>
                </div>
      ...
      <section className="mt-6">
        <SectionTitle title={t.rookies.draftBoard} sub={t.rookies.draftBoardSub(classYear)}
                ...
                <Link to={`/players/${p.playerId}`}>
                  <GlassCard hover sheen padded={false} className="relative overflow-hidden"
                    ...
                    <dl className="grid grid-cols-4 border-t text-center" style={{ borderColor: 'var(--border)' }}>
                      {([[t.abbr.pts, f.dec(p.pts), false], [t.abbr.reb, f.dec(p.reb), false],
                         [t.abbr.ast, f.dec(p.ast), false],
                         [t.rookies.roy, f.dec(roy.total), true]] as const).map(([k, v, isRoy]) => (
                        <div key={k} className="border-r px-1 py-1.5 last:border-0"
                          ...
                          <dd className="num text-sm font-bold"
                            style={{ color: isRoy ? team.primaryColor : undefined }}>{v}</dd>
                        </div>
                      ))}
                    </dl>
                    {sort !== 'pick' && (
      ...
      <section className="mt-6">
        <GlassCard>
          <SectionTitle title={t.rookies.classTable} sub={t.rookies.classTableSub} />
          <div className="overflow-x-auto">
            <table className="w-full min-w-[900px] text-sm">
              <thead>
                <tr className="border-b text-left" style={{ borderColor: 'var(--border)' }}>
                  {[t.abbr.pick, t.abbr.player, t.abbr.team, t.abbr.pos, t.abbr.from, t.abbr.ht,
                    t.abbr.min, t.abbr.pts, t.abbr.reb, t.abbr.ast,
                    t.abbr.ts, t.abbr.per, t.abbr.salary].map((h) => (
              ...
              <tbody>
                {[...rows].sort((a, b) => (a.bio.draftPick ?? 99) - (b.bio.draftPick ?? 99)).map((p) => (
                  <tr key={p.playerId} className="border-b last:border-0 hover-layer-1"
                    ...
                    <td className="num px-2 py-2">{f.dec(p.ts * 100, 1)}</td>
