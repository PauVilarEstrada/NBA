...
import type {
  ...
  PlayDescriptor, PricedPlayer, SeasonRow, SimEvent, SimResult, Team, TeamPrediction,
} from '@/types'
...
// ------------------------------------------------------------------- fantasy
/** Budgets are in real cap dollars, because prices are real cap dollars.
 *  The league salary cap is 154.6M, so $120M is a tight roster and $250M is a
 *  superteam you would never be allowed to assemble. */
export const BUDGET_PRESETS = [80_000_000, 120_000_000, 180_000_000, 250_000_000]
export const ROSTER_MIN = 5
export const ROSTER_MAX = 10

/**
 * A player costs what he is worth. Full stop.
 *
 * An earlier version rescaled every price to the chosen budget, so Jokić cost
 * "38% of whatever you picked" and changing the budget silently repriced the
 * league — which meant the roster had to be wiped every time, and no price ever
 * meant anything. Now the market value is the price, the budget is the only
 * thing you control, and picking a bigger budget simply lets you keep the team
 * you already built.
 */
export function pricePool(budget: number): PricedPlayer[] {
  return PLAYERS.map((p) => {
    const cost = p.marketPrice
    return {
      ...p,
      cost,
      costPct: Math.round((1000 * cost) / Math.max(budget, 1)) / 10,
      // production per million spent — the number that makes this a puzzle
      efficiency: Math.round(((p.pts + 1.2 * p.reb + 1.5 * p.ast) /
        Math.max(cost / 1_000_000, 0.5)) * 100) / 100,
    }
...
export function simulateGame(
  ...
  const emit = (team: SimT, kind: SimEvent['kind'], text: string,
                ...
                extra: { assist?: SimP; defender?: SimP; play?: PlayDescriptor } = {}) => {
    ...
    events.push({
      ...
      kind, text, play: extra.play, homeScore: home.score, awayScore: away.score,
  ...
  const rebound = (off: SimT, def: SimT, depth = 0) => {
    ...
    emit(team, 'rebound', `${board.name} ${offensive ? 'offensive' : 'defensive'} rebound`, board,
      0, false, { play: { t: 'rebound', name: board.name, offensive } })
  ...
  const secondChance = (off: SimT, def: SimT, board: SimP, depth: number) => {
    ...
    if (rng.next() < pMake) {
      ...
      emit(off, 'shot_made', `${actor.name} second-chance bucket`, actor, 2, putback,
        { play: { t: 'secondChance', actor: actor.name } })
    } else {
      emit(off, 'shot_miss', `${actor.name} misses the putback`, actor, 0, false,
        { play: { t: 'putbackMiss', actor: actor.name } })
  ...
  const possession = (off: SimT, def: SimT) => {
    ...
    if (rng.next() < BASE_TURNOVER * (1 + dStr * 2) * (actor.tovRate / 0.12)) {
      ...
      emit(off, 'turnover', `${actor.name} turnover, stolen by ${stealer.name}`, actor,
        0, false, { defender: stealer,
          play: { t: 'turnover', actor: actor.name, stealer: stealer.name } })
      return
    }
    ...
    if (!isThree && rng.next() < 0.055 + dStr) {
      ...
      emit(off, 'shot_miss', `${actor.name} ${kind} BLOCKED by ${blocker.name}`, actor, 0, true,
        { defender: blocker,
          play: { t: 'blocked', actor: actor.name, kind, blocker: blocker.name } })
      rebound(off, def)
      return
    }
    ...
    if (rng.next() < pMake) {
      ...
      if (rng.next() < 0.075) {
        actor.fta++
        if (rng.next() < 0.77) {
          actor.ftm++; actor.pts++; off.score++; text += ' — AND ONE!'; andOne = true
        }
      }
      emit(off, 'shot_made', text, actor, pts, isThree || kind === 'rim',
        { assist: passer,
          play: { t: 'made', actor: actor.name, kind, assist: passer?.name, andOne } })
      return
    }
    ...
    if (rng.next() < actor.ftRate * 0.5) {
      ...
      emit(off, 'ft', `${actor.name} ${made}/${shots} from the line`, actor, made, false,
        { play: { t: 'ft', actor: actor.name, made, shots } })
      return
    }

    emit(off, 'shot_miss', `${actor.name} misses the ${kind}`, actor, 0, false,
      { play: { t: 'miss', actor: actor.name, kind } })
    rebound(off, def)
  }
  ...
  for (;;) {
    ...
    elapsed = end
    const periodLabel = `${period <= 4 ? 'Q' : 'OT'}${period <= 4 ? period : period - 4}`
    emit(home, 'period', `End of ${periodLabel}`, undefined, 0, false,
      { play: { t: 'period', label: periodLabel } })
