              ...
              <div className="min-w-0">
                <p className="flex items-baseline gap-1.5">
                  ...
                  <span className="truncate text-[11px] font-bold uppercase tracking-wider">
                    {t.honour[h.label as keyof typeof t.honour] ?? h.label}
                  </span>
                </p>
  ...
  if (!titles.length && !aba.length) {
    return (
      <div className="rounded-xl px-4 py-8 text-center" style={{ background: 'var(--layer-1)' }}>
        <p className="headline text-2xl" style={{ color: 'var(--text-muted)' }}>{t.team.raftersEmpty}</p>
        <p className="mt-1 text-xs" style={{ color: 'var(--text-muted)' }}>
          {t.team.raftersEmptyHint(franchise.founded ?? null)}
        </p>
      </div>
    )
  }
    ...
    <div>
      <div className="flex flex-wrap gap-2">
        {titles.map((year, i) => (
          <motion.div key={year}
            ...
            title={franchise.notes?.[String(year)] ?? t.team.championsTitle(abbr)}
      ...
      {aba.length > 0 && (
        <p className="mt-3 text-[11px]" style={{ color: 'var(--text-muted)' }}>
          {t.team.abaNote(aba.length, aba.join(', '))}
        </p>
      )}
