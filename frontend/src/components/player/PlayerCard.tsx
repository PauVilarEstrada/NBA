        ...
        >
          ...
          <div className="relative flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              {/* TODO i18n: no dictionary key for the "<money> market value" line yet. */}
              <p className="mt-1 text-xs" style={{ color: 'var(--text-muted)' }}>
                {f.money(player.marketPrice)} market value
              </p>
            </div>
          </div>

          <dl className="relative mt-4 grid grid-cols-4 gap-1.5">
            {([['pts', t.abbr.pts, player.pts], ['reb', t.abbr.reb, player.reb],
               ['ast', t.abbr.ast, player.ast],
               ['ts', t.abbr.ts, player.ts * 100]] as const).map(([k, label, v]) => (
