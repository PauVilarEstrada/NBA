  ...
  return (
    <div className="glass relative overflow-hidden rounded-2xl px-6 py-16 text-center">
      ...
      <div className="relative">
        <p className="eyebrow">{isPlayoffs ? t.common.playoffGame : t.common.regularSeason}</p>

        <div className="mt-4 flex items-center justify-center gap-6 sm:gap-12">
          <SideBadge abbr={awayAbbr} teamId={awayId} label={t.common.away} />
          <div className="relative">
            <motion.span
          ...
          <SideBadge abbr={homeAbbr} teamId={homeId} label={t.common.home} />
        </div>

        <p className="headline mt-8 text-3xl">{t.sim.tipOff}</p>
        <p className="mt-1 text-sm" style={{ color: 'var(--text-2)' }}>{stageLabel(stage, t)}…</p>
...
/**
 * The simulation runs in a Web Worker, which has no dictionary, so it reports
 * the stage it has reached as a fixed English token. The token is turned back
 * into copy here, where the active locale is known.
 */
function stageLabel(stage: string, t: Dict): string {
