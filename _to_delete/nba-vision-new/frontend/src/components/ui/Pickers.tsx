...
export function PlayerPicker({
  value, onChange, placeholder = 'Search a player…', exclude = [],
}: {
  value: Player | null
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="block text-xs" style={{ color: 'var(--text-2)' }}>
              {value.team} · {value.position} · {value.pts.toFixed(1)} PPG
            </span>
          </span>
          <span className="shrink-0 text-xs" style={{ color: 'var(--text-muted)' }}>change</span>
        </button>
      ) : (
        <input
          value={query}
            ...
            {results.map((p, i) => (
              <li key={p.playerId}>
                ...
                >
                  ...
                  <span className="num text-xs font-bold">{p.pts.toFixed(1)}</span>
                </button>
              </li>
            ))}
          </motion.ul>
