...
export function archetype(p: Player): { label: string; detail: string } {
  ...
  if (creator && scorer) return { label: 'Primary engine', detail: 'Carries the offence as both scorer and creator' }
  if (creator) return { label: 'Lead playmaker', detail: 'High usage, distributes more than he finishes' }
  if (scorer && shooter) return { label: 'Movement shooter', detail: 'Scoring volume built on three-point gravity' }
  if (scorer) return { label: 'Bucket getter', detail: 'Elite scoring volume, moderate playmaking load' }
  if (rim) return { label: 'Rim protector', detail: 'Anchors the paint on both glass and shot-blocking' }
  if (shooter) return { label: 'Floor spacer', detail: 'Low usage, high three-point volume and accuracy' }
  if (stopper) return { label: 'Perimeter stopper', detail: 'Defensive event creation well above his usage' }
  if (bigMan) return { label: 'Connective big', detail: 'Screens, rebounds and finishes inside the offence' }
  return { label: 'Role wing', detail: 'Complementary scoring inside a defined role' }
}

/**
 * A written scouting report, generated from the numbers.
 *
 * Every sentence is derived from a percentile threshold — nothing here is
 * invented prose. It is a readable view of the same data the charts show, which
 * is what makes it useful rather than decorative.
 */
export function scoutingReport(p: Player): { headline: string; strengths: string[]; concerns: string[]; summary: string } {
  const q = p.percentiles
  const team = engine.getTeam(p.teamId)
  const arch = archetype(p)
  const strengths: string[] = []
  const concerns: string[] = []

  if (q.pts >= 90) strengths.push(`Scoring volume in the top ${100 - q.pts + 1}% of the league at ${p.pts.toFixed(1)} a night.`)
  if (q.ts >= 80) strengths.push(`Elite efficiency — ${(p.ts * 100).toFixed(1)}% true shooting on a ${(p.usg * 100).toFixed(1)}% usage rate.`)
  if (q.ast >= 80) strengths.push(`Creates for others at a ${q.ast}th-percentile rate (${p.ast.toFixed(1)} assists).`)
  if (q.reb >= 82) strengths.push(`Controls the glass: ${p.reb.toFixed(1)} rebounds, ${q.reb}th percentile.`)
  if (q.stl >= 80 || q.blk >= 80) strengths.push(`Generates defensive events — ${p.stl.toFixed(1)} steals and ${p.blk.toFixed(1)} blocks.`)
  if (q.fg3Pct >= 78) strengths.push(`A genuine floor spacer at ${(p.shooting.fg3Pct * 100).toFixed(1)}% from three.`)
  if (!strengths.length) strengths.push(`Fills a defined role without a standout statistical strength.`)

  if (q.ts <= 30) concerns.push(`Efficiency lags at ${(p.ts * 100).toFixed(1)}% true shooting — bottom third of the league.`)
  if (q.tov <= 25) concerns.push(`Turns it over ${p.tov.toFixed(1)} times a game for his usage.`)
  if (q.fg3Pct <= 25 && (p.shooting.fg3a ?? 0) > 2) concerns.push(`Takes ${p.shooting.fg3a.toFixed(1)} threes a game at only ${(p.shooting.fg3Pct * 100).toFixed(1)}%.`)
  if (q.reb <= 20 && (p.position === 'PF' || p.position === 'C')) concerns.push(`Rebounds below what his position usually provides.`)
  if (p.age >= 34) concerns.push(`Age ${p.age} — the aging curve is working against the contract from here.`)
  if (p.surplus < 0) concerns.push(`Paid above the model's valuation by ${Math.abs(Math.round(p.surplus / 1e6))}M.`)
  if (!concerns.length) concerns.push(`No material statistical red flags this season.`)

  const headline = `${arch.label} · ${p.position} · ${team?.abbr ?? ''}`
  const summary =
    `${p.name} profiles as a ${arch.label.toLowerCase()} — ${arch.detail.toLowerCase()}. ` +
    `He is producing ${p.pts.toFixed(1)} points, ${p.reb.toFixed(1)} rebounds and ${p.ast.toFixed(1)} assists ` +
    `in ${p.min.toFixed(1)} minutes at ${(p.ts * 100).toFixed(1)}% true shooting, ` +
    `which puts him in the ${q.per}th percentile on PER and the ${q.vorp}th on VORP. ` +
    (team ? `${team.fullName} are ${team.wins}-${team.losses} with a ${team.netRating > 0 ? '+' : ''}${team.netRating.toFixed(1)} net rating.` : '')

  return { headline, strengths, concerns, summary }
}
