...
export function PageLoader({ label = 'Loading', sub }: { label?: string; sub?: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-28 text-center" role="status" aria-live="polite">
      <BallSpinner />
      <p className="headline mt-6 text-2xl">{label}</p>
  ...
  render() {
    ...
    return (
      <div className="glass mx-auto my-16 max-w-xl rounded-2xl p-8 text-center">
        <p className="headline text-3xl">Turnover</p>
        <p className="mt-2 text-sm" style={{ color: 'var(--text-2)' }}>
          {this.props.fallbackLabel ?? 'Something in this view failed to render.'}
        </p>
