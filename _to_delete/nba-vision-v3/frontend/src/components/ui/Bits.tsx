...
export function SourceBadge({ source }: { source: 'model' | 'mock' }) {
  return source === 'model'
    ? <Badge tone="good">Trained model</Badge>
    : <Badge tone="brand" className="cursor-help"
        title="No trained artefact loaded: this projection comes from the calibrated baseline model (real form, real opponent ratings, real home/rest effects — no learned weights).">
        Baseline model
      </Badge>
}
