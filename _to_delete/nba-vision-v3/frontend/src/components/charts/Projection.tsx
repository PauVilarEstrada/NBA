      ...
      <div className="relative h-7">
        ...
        {dist.line !== undefined && (
          <div className="absolute top-0 h-full border-l border-dashed border-[var(--text-muted)]"
            ...
            title={`Line ${dist.line}`} />
        )}
      </div>

      <div className="mt-1 flex justify-between text-[10px]" style={{ color: 'var(--text-muted)' }}>
        <span className="num">{p05.toFixed(1)}</span>
        <span>80% of outcomes between <b className="num text-[var(--text-2)]">{dist.low.toFixed(1)}</b>
          {' '}and <b className="num text-[var(--text-2)]">{dist.high.toFixed(1)}</b></span>
        <span className="num">{p95.toFixed(1)}</span>
      </div>
  ...
  return (
    <div className="mt-3 flex items-center gap-2">
      ...
      <div className="flex h-5 flex-1 overflow-hidden rounded-md" role="img"
        aria-label={`Over ${over.toFixed(1)} percent, under ${(100 - over).toFixed(1)} percent`}>
        <motion.div
        ...
        <div className="flex flex-1 items-center justify-end pr-1.5 text-[10px] font-bold"
          ...
          {100 - over >= 22 && `UNDER ${(100 - over).toFixed(0)}%`}
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div>
      <div className="mb-1.5 flex items-baseline justify-between text-xs font-bold uppercase tracking-wider">
        <span style={{ color: homeColor }}>{homeAbbr} <span className="num">{h.toFixed(1)}%</span></span>
        <span style={{ color: awayColor }}><span className="num">{(100 - h).toFixed(1)}%</span> {awayAbbr}</span>
      </div>
          ...
          <li key={d.label}>
            <div className="flex items-baseline justify-between gap-2 text-xs">
              ...
              <span className="num font-bold"
                ...
                {positive ? '+' : ''}{d.impact.toFixed(2)} {unit}
              </span>
            </div>
