...
3. **Attendance and ticket prices are modelled.** Arena name, capacity, opening
   year and location are facts (`app/services/arena.py`). The gate is modelled
   from capacity, win rate, market size, arena age and the visiting team's draw,
   then anchored to the public league averages — about 18,300 a night at roughly
   95% of capacity in 2024-25. The NBA publishes no free per-game attendance
   feed and resale pricing sits behind paid APIs (Ticketmaster and StubHub
   partner programmes); connect one through the ingest job and these fields fill
   from source. Every panel that shows them says they are modelled.
4. **Player ids for the most recent draft classes are approximate.** Ids for
