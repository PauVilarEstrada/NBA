...
from app.services import arena, bios, franchise
...
def all_teams() -> list[dict]:
    ...
    for abbr, (off, deff, pace, w, l) in TEAM_RATINGS.items():
        ...
        t.update({
            ...
            "venue": arena.attendance_for(abbr, w / (w + l), off - deff),
            "tickets": arena.ticket_market(abbr, w / (w + l), off - deff),
        })
        out.append(t)

    # Anchor the modelled gate to the real league average (~18,300 a night in
    # 2024-25). Without this the formula's absolute level is arbitrary; with it
    # only the *spread* between teams is modelled, which is the honest part.
    raw_mean = sum(t["venue"]["averageAttendance"] for t in out) / len(out)
    scale = arena.LEAGUE_AVG_ATTENDANCE / raw_mean
    for t in out:
        v = t["venue"]
        v["averageAttendance"] = int(round(v["averageAttendance"] * scale))
        v["fillRate"] = round(min(v["averageAttendance"] / v["capacity"], 1.005), 3)
        v["totalAttendance"] = v["averageAttendance"] * 41
        v["vsLeagueAvg"] = v["averageAttendance"] - arena.LEAGUE_AVG_ATTENDANCE
        t["recentHomeGames"] = arena.recent_home_games(
            t["abbr"], t["winPct"], t["netRating"],
            [x for x in TEAM_RATINGS if x != t["abbr"]][:8])
    ...
    for key, higher in (("offRating", True), ("defRating", False), ("netRating", True),
        ...
        for i, t in enumerate(ordered, 1):
            t[f"{key}Rank"] = i
    for i, t in enumerate(sorted(out, key=lambda x: -x["venue"]["averageAttendance"]), 1):
        t["attendanceRank"] = i
    for i, t in enumerate(sorted(out, key=lambda x: -x["tickets"]["averagePrice"]), 1):
        t["ticketPriceRank"] = i
    return out
