...
# Budgets are real cap dollars, because prices are real cap dollars. The
# 2025-26 cap is 154.6M, so 80M is a shoestring and 250M is a superteam no front
# office would be allowed to assemble.
BUDGET_PRESETS = [80_000_000, 120_000_000, 180_000_000, 250_000_000]
ROSTER_MIN, ROSTER_MAX = 5, 10


def price_pool(budget: int) -> list[dict]:
    """The whole league at its own market value.

    Prices do not depend on the budget. An earlier version rescaled every price
    so the most expensive player always cost a fixed share of whatever budget
    you picked — which made the numbers meaningless and forced the roster to be
    wiped every time the budget changed.
    """
    out = []
    for p in catalog.all_players():
        ...
        out.append({
            **{k: p[k] for k in ("playerId", "name", "team", "teamId", "position",
                                 ...
                                 "stl", "blk", "tov", "fg3m", "ts", "usg", "min",
                                 "salary", "per", "vorp")},
            "cost": cost,
            "costPct": round(100 * cost / max(budget, 1), 1),
            "marketPrice": cost,
            "valueIndex": p["valueIndex"],
            # production per million spent — the stat that makes the mode a puzzle
            "efficiency": round((p["pts"] + 1.2 * p["reb"] + 1.5 * p["ast"])
                                / max(cost / 1_000_000, 0.5), 2),
        })
